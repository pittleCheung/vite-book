...
1.  **多核并行**。内部打包算法充分利用多核 CPU 优势，所有的步骤尽可能并行，这也是得益于 Go 当中多线程共享内存的优势。
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a6f4b8a7d64d4f06880f84fa3a67325d~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/741e90b443534c71b139bbe00c555be1~tplv-k3u1fbpfcp-zoom-1.image)

以上就是 Esbuild 打包的元信息，这对我们编写插件扩展 Esbuild 能力非常有用。

接着，我们再观察一下 dist 目录，发现打包产物和相应的 SourceMap 文件也已经成功写入磁盘:

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c3ae8300954548d49791a58706c3d41e~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  类似 webpack-dev-server，所有的产物文件都默认不会写到磁盘，而是放在内存中，通过请求服务来访问。
1.  **每次请求**到来时，都会进行重新构建(`rebuild`)，永远返回新的产物。
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d297eaed4b4f43fb936aa41895f202b5~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  onEnd 钩子中如果要拿到  `metafile`，必须将 Esbuild 的构建配置中`metafile`属性设为  `true`。
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dbee5e517a794c26a09a59ecd4ca4c63~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2be714d48f46498db6023109d4bb81ca~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b4383510ecd3448ba845c4eb10656e58~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d1bc1bb213314834ab34deb2f16dcd5d~tplv-k3u1fbpfcp-zoom-1.image)

现在我们再次执行`node build.js`，发现依赖已经成功下载并打包了。

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/305407c6b5514c5bba5f2196faf2913b~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bc8d913f6bdc4a02a13b2bee4a14a2ed~tplv-k3u1fbpfcp-zoom-1.image)

再访问`localhost:3000`，会默认访问到 index.html 的内容：

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9fc37948cd9344be9d5b654787540fb9~tplv-k3u1fbpfcp-zoom-1.image)

这样一来，应用的内容就成功显示了，也说明 HTML 插件正常生效了。当然，如果要做一个足够通用的 HTML 插件，还需要考虑诸多的因素，比如`自定义 HTML 内容`、`自定义公共前缀(publicPath)`、`自定义 script 标签类型`以及  `多入口打包`等等，大家感兴趣的话可以自行扩展。(可参考[这个开源插件](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fsanyuan0704%2Fewas%2Fblob%2Fmain%2Fpackages%2Fesbuild-plugin-html%2Fsrc%2Findex.ts "https://github.com/sanyuan0704/ewas/blob/main/packages/esbuild-plugin-html/src/index.ts"))
